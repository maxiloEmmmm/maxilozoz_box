...
class MinRoute {
  ...
  MinRouteGroupModel group = MinRouteGroupModel(using: false, pre: '', middlewares: []);

  Route<dynamic> generate(RouteSettings settings){
    var route = this.match(settings.name);

    if(route != null) {
      RouteSettings _settings = routeMiddleware.filter(settings.copyWith(), route.middlewares);
      ...
      if(_settings.name == settings.name) {
        ...
        try {
          ...
          //todo: err page
          print("[todo] maxilozoz route: param function or no param function not exist.");
        }
        
        return MaterialPageRoute(builder: (BuildContext _) => widget, settings: settings);
      }else {
        settings = _settings;
      }
    }
  ...
  bool add(String path, Function make, {List middlewares}){
    ...
    if(this.group.using) {
      ...
      if(this.group.middlewares.length > 0) {
        middlewares = ((middlewares ?? []) + this.group.middlewares).toSet().toList();
      }
    }
